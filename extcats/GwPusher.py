...
def map_to_feature_collection(gw_localization_map, confidence_levels=0.1):
    ...
    if not fc.is_valid:
        raise ValueError(
            "input string %s is not valid geoJSON. Errors: %s"
...
def feature_to_polygons(feature):
    ...
    if not mls.is_valid:
        raise ValueError(
            "cannot convert feature geometry %s to valid geojson.MultiLineString"
...
class GwPusher:
    ...
    def __init__(self, dbname, coll_name, dbclient=None, logger=None):
        ...
        self.logger.info(
            "connected to collection %s of database %s" % (self.dbname, self.coll_name)
        )
        self.logger.debug(f"using mongo client at {self.dbclient.address}")
        self.logger.debug(
            "database stats: %s" % json.dumps(self.db.command("dbstats"), indent=2)
        )

    def insert_gw_contours(
        self,
        gw_localization_map,
        confidence_levels,
        gw_id=None,
        ...
        **kwargs,
        ...
        self.logger.info(
            "inserting contours at %s CL for %s in collection %s."
            % (
                gw_localization_map,
                repr(confidence_levels),
                ".".join([self.db.name, self.coll.name]),
            )
        )
        ...
        for feature in fc.features:
            ...
            for poly in polygons:
                ...
                buff = {
                    ...
                    "CL": cl,
                    self.geom_key: {"type": "Polygon", "coordinates": poly.coordinates},
                }
                docs.append(buff)
        self.logger.debug(
            "extracted %d documents from GW map %s" % (len(docs), gw_localization_map)
        )
        ...
        self.logger.info(
            "inserted %d documents (out of %d) into the database."
    ...
    def polygon_meta(self):
        ...
        self.logger.info(
            "adding metadata for geoJSON Polygon field named: %s" % self.geom_key
        )
        self.db["meta"].update(
            {"_id": self.geom_key},
            {"_id": self.geom_key, "key": self.geom_key, "type": "geoJSON.Polygon"},
            upsert=True,
        )
