...
class GwQuery:
    ...
    def __init__(self, db_name, coll_name="srcs", dbclient=None, logger=None):
        """
        ...
        self.logger.info(
            "connected to collection %s of database %s. %s documents in collection."
    ...
    def check_polygon(self):
        ...
        if len(geo_doc) == 1:
            ...
            self.logger.debug(
                "found geoJSON.Polygon key %s in metadata collection" % (self.poly_key)
            )
        elif len(geo_doc) == 0:
            self.poly_key = None
            ...
            self.logger.warning(
                "more than one geoJSON.Polygon key in metadata collection. Use set_polygon_key()."
            )
            self.poly_key = None
