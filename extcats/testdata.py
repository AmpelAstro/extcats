...
class MQC:
    def download(
        datadir="./testdata/milliquas",
        fname="heasarc_milliquas.tdat.gz",
        overwrite=False,
    ):
        ...
        if (not os.path.isfile(dest)) and (not overwrite):
            urlretrieve(
                "https://desycloud.desy.de/index.php/s/AsxkEJUJ8LrgJkd/download", dest
            )
...
class PS1Small:
    def download(datadir="./testdata/PS1DR1_test", overwrite=False):
        ...
        if not os.listdir(datadir) and (not overwrite):
            ...
            urlretrieve(
                "https://desycloud.desy.de/index.php/s/3AAJ30J7YZ7dH43/download", tmp
            )
            datadir_parent = os.path.abspath(os.path.join(datadir, os.pardir))
            unpack_archive(tmp, datadir_parent)
        else:
            print(
                "found test PS1DR1 data in %s. Use overwrite to re-download it."
                % datadir
            )
        return datadir
