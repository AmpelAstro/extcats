...
def searcharound_HEALPix(
    ...
    while pixgroup_scale / 2.0 < rs_arcsec:
        pix_ras, pix_decs = pix2ang(nside, pix_group, lonlat=True, nest=hp_nest)
        new_pixels = get_all_neighbours(
            nside, pix_ras, pix_decs, nest=hp_nest, lonlat=True
        )
    ...
    qresults = query_and_return(
        combined_filter, projection, src_coll, find_one, to_table=False
    )
    ...
    if not circular:
        ...
        if (ra_key not in cols or dec_key not in cols) and sphere2d_key is not None:
            ...
            tab[ra_key], tab[dec_key] = zip(
                *(q[sphere2d_key]["coordinates"] for q in qresults)
            )
...
def searcharound_RAW(
    ...
    query_func = Code(
        ...
        % (
            ...
            dec,
            ra,
            radians(rs_arcsec / 3600.0),
        )
    )

    # restrict first to a box in the query
    box_size = box_scale * (rs_arcsec / 3600.0)
    qfilter = {
