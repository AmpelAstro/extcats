...
class CatalogPusher:
    ...
    def __init__(
        self, catalog_name, data_source, file_type=None, recursive=True, logger=None
    ):
        """
        Parameters:
        -----------
        ...
        if isinstance(data_source, str):
            ...
            raise OSError(
                "data_source param must be string or lits, got %s instead"
                % type(data_source)
            )
        ...
        if len(self.raw_files) == 0:
            self.logger.warning(
                "no data found for catalog %s in source: %s"
                % (catalog_name, data_source)
            )
        else:
            self.logger.info(
                "found %d files for catalog %s in data source: %s"
    ...
    def check_raw_files(self):
        ...
        if len(types_set) > 1:
            raise TypeError(
                "inconsistent file types %s in raw_files: \n%s.\nUse file_type parameter in constructor."
                % (", ".join(types_set), "\n".join(self.raw_files))
            )
    ...
    def assign_file_reader(self, reader_func, read_chunks, **reader_args):
        ...
        self.logger.info(
            "file reader %s assigned to pusher." % (self.file_reader.__name__)
        )
    ...
    def assign_dict_modifier(self, modifier_func):
        ...
        self.logger.info(
            "source document modifer %s assigned to pusher."
    ...
    def insert_file_todb(
        ...
        if not dry:
            self.logger.info(
                "inserted %d documents in %.2e seconds" % (tot_docs, end - start)
            )
        else:
            self.logger.info(
                "DRY RUN: parsed %d documents in %.2e seconds" % (tot_docs, end - start)
            )
        return raw_file
    ...
    def push_to_db(
        ...
        if not dry:
            ...
            if coll_name in db.list_collection_names() and overwrite_coll:
                self.logger.warning(
                    "overwrite_coll asserted: collection %s will be dropped."
                    % coll_name
                )
            ...
            if coll_name in db.list_collection_names() and (not append_to_coll):
                self.logger.warning(
                    "collection %s already exists and append_to_coll is False. Nothing to do."
                    % coll_name
                )
                return
            ...
            self.logger.info(
                "collection has the following indexes: %s"
    ...
    def info(self):
        ...
        self.logger.info(
            "here are some statis on database %s and collection %s"
    ...
    def sphere2d_meta(self, sphere2d_key, is_indexed, pos_format="geoJSON"):
        ...
        self.logger.info(
            "adding metadata for 2dsphere coordinates named: %s" % sphere2d_key
        )
        self.db["meta"].update_one(
            {"_id": sphere2d_key},
            {
