...
from extcats.catquery_utils import (
    searcharound_HEALPix,
    searcharound_9HEALPix,
    searcharound_2Dsphere,
    searcharound_RAW,
    get_closest,
)
...
class CatalogQuery:
    ...
    def __init__(
        ...
        self.logger.info(
            "connected to collection %s of database %s. %s documents in source collection %s."
            % (
                coll_name,
                self.cat_db.name,
                self.src_coll.estimated_document_count(),
                coll_name,
            )
        )
    ...
    def check_healpix(self):
        ...
        if len(hp_docs) == 0:
            self.logger.debug(
                "no HEALPix description found in metadata for catalog %s"
                % self.cat_db.name
            )
        if len(hp_docs) == 1 and hp_docs[0]["is_indexed"]:
            ...
            self.logger.debug(
                "found %d HEALPix partitions for this catalog." % len(hp_docs)
            )
            for i, _ in enumerate(hp_docs):
                self.logger.info("#%d %s" % (i, repr(_)))
        ...
        if not hp_doc is None:
            ...
            self.hp_resol = nside2resol(2**self.hp_order, arcmin=True) * 60.0
            self.logger.info(
                "set HEALPIX partition of order %d with key '%s'. Nested: %s, Indexed: %s, Resolution [\"]: %.3f"
                % (
                    self.hp_order,
                    self.hp_key,
                    str(self.hp_nest),
    ...
    def check_sphere2d(self):
        ...
        if len(sphere2d_doc) == 0:
            ...
            self.logger.info(
                "set 2dsphere key '%s' with format %s. Indexed: %s"
    ...
    def autoset_method(self):
        ...
        if self.sphere2d_index:
            ...
            self.logger.warning(
                "no HEALPix / 2dsphere index found. Queries will be SUPER SLOW!"
            )
            self.default_method = "healpix"
    ...
    def findwithin_HEALPix(
        self, ra, dec, rs_arcsec, circular=True, find_one=False, **qfunc_args
    ):
        """
        ...
        return searcharound_HEALPix(
            ...
            dec=dec,
            rs_arcsec=rs_arcsec,
            src_coll=self.src_coll,
            hp_key=self.hp_key,
            hp_order=self.hp_order,
            hp_nest=self.hp_nest,
            hp_resol=self.hp_resol,
            circular=circular,
            ra_key=self.ra_key,
            dec_key=self.dec_key,
            sphere2d_key=self.s2d_key,
            find_one=find_one,
            pre_filter=qfunc_args.get("pre_filter"),
    ...
    def findwithin_9HEALPix(self, ra, dec, find_one=False, **qfunc_args):
        ...
        if not self.has_hp:
            raise RuntimeError("catalog has no healpix index. Cannot use it to query.")
        self.logger.warning(
            "queries sources in a 9-pixel square of %.3f arcsec side around target"
            % (3 * self.hp_resol)
        )
    ...
    def findwithin_2Dsphere(self, ra, dec, rs_arcsec, find_one=False, **qfunc_args):
        ...
        if not self.has_2dsphere:
            raise RuntimeError(
                "catalog has no geoJSON/legacy pair object. Cannot use it to query."
            )
    ...
    def findwithin_RAW(
        self, ra, dec, rs_arcsec, box_scale=2.0, find_one=False, **qfunc_args
    ):
        """
        Returns sources in catalog within rs_arcsec from target position.
    ...
    def findwithin(self, ra, dec, rs_arcsec, method=None, **qfunc_args):
        ...
        if method == "healpix":
            ...
            raise ValueError(
                "invalid method parameter: %s. Valid ones are 'healpix'/'2dsphere'/'raw'"
                % method
            )
    ...
    def test_queries(
        ...
        **qfunc_args,
        ...
        if points is None:
            ...
            npoints = len(points)
            self.logger.info(
                "running test queries using %d user defined points" % (npoints)
            )
        ...
        self.logger.info(
            "running %d test queries using function: %s and method: %s"
        ...
        self.logger.info(
            "Took %.2e sec for %d random queries. Average query time: %.3e sec\n"
